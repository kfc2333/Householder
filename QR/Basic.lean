...
theorem diff_gamma_bound_trans (n₁ n₂ : ℕ) (x x' x'' : ℝ) (hnu : ((n₁ + n₂) : ℝ) * (u : ℝ) < 1) (hbound₁ : diff_gamma_bound u n₁ x x') (hbound₂ : diff_gamma_bound u n₂ x' x'') :
  ...
  · rcases hbound₁ with ⟨θ₁, hxx', hθ₁⟩
    ...
    · calc
      ...
      _ = (γ u n₁ + 1) * (γ u n₂ + 1) - 1 := by ring
      _ ≤ _ := by linarith [gamma_mul_le u n₁ n₂ hnu]
...
lemma mul_bound
  ...
  have hnuy : (ny : ℝ) * (u : ℝ) < 1 := by
    ...
    _ ≤ ((nx + ny) : ℝ) * (u : ℝ) := by
      ...
      · exact u.2
    _ < _ := hnu
  rw [← diff_gamma_bound_theta_iff_diff_gamma_bound, diff_gamma_bound_theta] at hboundx hboundy ⊢
  · rcases hboundx with ⟨θ₁, hxx', hboundx⟩
    rcases hboundy with ⟨θ₂, hyy', hboundy⟩
    exists (θ₁ + θ₂ + θ₁ * θ₂)
    constructor
    · rw [hxx', hyy']
      ring_nf
    · calc
        ...
        _ = |θ₁| + |θ₂| + |θ₁| * |θ₂| := by
          rw [abs_mul]
        _ ≤ γ u nx + γ u ny + γ u nx * γ u ny := by
          gcongr
          exact gamma_nonneg u nx hnux
        _ = (γ u nx + 1) * (γ u ny + 1) - 1 := by ring
        _ ≤ _ := by linarith [gamma_mul_le u nx ny hnu]
  · exact_mod_cast hnu
  · exact_mod_cast hnuy
  · exact_mod_cast hnux
...
lemma fl_add_bound'
  ...
  diff_gamma_bound u ((nx ⊔ ny) + 1) (x + y) z := by
    ...
    have h1 : diff_gamma_bound u 1 (x' + y') z := by
      ...
      · exact hfl_add
    have h2 : diff_gamma_bound u (nx ⊔ ny) (x + y) (x' + y') := by
      exact add_bound_nonpos u nx ny x y x' y' hx hy hnu' hboundx hboundy
    exact diff_gamma_bound_trans u (nx ⊔ ny) 1 (x + y) (x' + y') z (by simpa only [Nat.cast_one] using hnu) h2 h1
lemma fl_mul_bound
  (nx ny : ℕ)
  (x y x' y' z : ℝ)
  ...
  diff_gamma_bound u ((nx + ny) + 1) (x * y) z := by
    ...
    have h1 : diff_gamma_bound u 1 (x' * y') z := by
      ...
      · exact hfl_mul
    have h2 : diff_gamma_bound u (nx + ny) (x * y) (x' * y') := by
      exact mul_bound u nx ny x y x' y' hnu' hboundx hboundy
    exact diff_gamma_bound_trans u (nx + ny) 1 (x * y) (x' * y') z (by simpa only [Nat.cast_add, Nat.cast_one] using hnu) h2 h1
