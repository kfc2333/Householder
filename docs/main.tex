...
{\par\noindent\textbf{#1.}\ \begingroup\bfseries}
...
\begin{definition}
	记$u$为上一章中提到的$u$，表示浮点计算模型精度：
\end{definition}
\begin{leancode}
variable (u : NNReal)
\end{leancode}

定义$$\gamma_n=\frac{nu}{1-nu}$$
\begin{leancode}
def γ (n : ℕ) : ℝ := (n : ℝ) * (u : ℝ) / (1 - (n : ℝ) * (u : ℝ))
\end{leancode}

为n次浮点舍入后计算结果与正确结果的相对误差。书中为避免对常数的讨论，还定义了$\widetilde{\gamma_n}=\gamma_{cn}$。本文由于在所有需要使用这个参数的地方都已经将常数显式计算出来，因此不再使用此参数。
