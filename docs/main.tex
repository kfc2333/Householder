...
lemma pos_diff_gamma_bound_pos (n : ℕ) (x x' : ℝ) 
  ...
  (hbound : diff_gamma_bound u n x x') :
    x' > 0 
\end{leancode}
$\gamma_n<1$时浮点误差不会导致被计算结果的正负号改变。
...
def fl_vec_dot : {n : ℕ} → Vec n → Vec n → ℝ → Prop
  ...
  | n + 1, x, y, z =>
    ∃ p s : ℝ,
      ...
      fl_add u p s z
\end{leancode}
定义了带误差的向量点乘。我们递归地定义这个过程。当$n=0$时，点乘结果为$0$。当$n>0$时，我们先计算第一个分量的乘积$p$，再递归计算剩余分量的点乘结果$s$，最后将两者相加得到最终结果$z$。每一步运算都满足一次浮点误差界。
...
\end{leancode}
向量内积自身的计算结果在$n+1$阶误差界内。与原书中第3.1章得到的结论不同，我们将条件修改为向量内积自身（在下面的证明中也只需要用到这种内积），而不是任意的两个向量点乘。从而我们可以得到更强的、直接关于计算得数的误差界。
\begin{proof}
  记$s_i=x_1x_1+\cdots+x_ix_i$。我们有
  \begin{align*}
    \hat{s_1}&=fl(0+fl(x_1x_1))\\
    &=fl(x_1x_1)(1+\theta_1)\\
    &=x_1x_1(1+\theta_2)\\
    \hat{s_i}&=fl(\hat{s_{i-1}}+fl(x_ix_i))\\
    &=(\hat{s_{i-1}}+x_ix_i(1+\theta_1))(1+\theta_1)\\
    &=(s_{i-1}(1+\theta_{i})+x_ix_i(1+\theta_1))(1+\theta_1)\\
    &=(s_{i-1}+x_ix_i)(1+\theta_{i+1})\\
    &=s_i(1+\theta_{i+1})
  \end{align*}
  即得$\hat{s_n}=s_n(1+\theta_{n+1})$。
\end{proof}
